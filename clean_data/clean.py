...
for row in sheet.iter_rows(min_row=2):  
    ...
    client = row[4].value
    LATITUDE_SITE_init = row[8].value
    ...
    dateAff_STT = row[16].value if len(row) > 16 else None 
    ...
    delegation = row[11].value if len(row) > 11 else None 
    gouvernorat = row[12].value if len(row) > 12 else None 
    delegation_clean = verifierDelegation(delegation)
    gouvernorat_clean = verifierGouv(gouvernorat)
    verifSTT = verif_Nom_STT(STT)
    offre = row[13].value if len(row) > 13 else None 
    pack = row[14].value if len(row) > 14 else None 
    OPENING_DATE_SUR_TIMOS = row[2].value if len(row) > 2 else None 
    STATUT = row[3].value if len(row) > 3 else None 
    REP_RDV = row[17].value if len(row) > 17 else None 
    DATE_PRISE_RDV = row[18].value if len(row) > 18 else None 
    CMT_RDV = row[22].value if len(row) > 22 else None 
    METRAGE_CABLE = row[28].value if len(row) > 28 else None 
    METRAGE_CABLE = verif_cable(METRAGE_CABLE)
    ...
    if len(row) > 2:
        latitude = clean_coordinate(row[8].value)
        longitude = clean_coordinate(row[9].value)
        ...
        coordinate = clean_coordinate(row[8].value)
...
try:
    new_file_pathError = './Erroned_data.xlsx'
...
try:
    new_file_path = './cleaned_data.xlsx'
