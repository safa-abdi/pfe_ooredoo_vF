...
def clean_crm_case(crm_case):
    ...
    if('ERROR_CASE_' in str(crm_case)):
        crm_case_cl = str(crm_case).strip('ERROR_CASE_')
        #print("crm case",crm_case)
        return crm_case_cl
    elif(str(crm_case) == 'ERROR_CASE'  ):
...
# Ajout des en-têtes
headers = ['CRM_CASE', 'DATE_CREATION_CRM', 'LATITUDE_SITE', 'LONGITUDE_SITE', 'MSISDN', 'CONTACT_CLIENT', 'CLIENT' ,'STT']
...
for data in Erroned_data:
    new_sheetError.append([data['CRM_CASE'], data['DATE_CREATION_CRM'], data['LATITUDE_SITE'], 
                           data['LONGITUDE_SITE'], data['MSISDN'], data['CONTACT_CLIENT'], data['CLIENT'], data['STT']])
...
try:
    new_file_path = './cleaned_data_Resiliation.xlsx'
