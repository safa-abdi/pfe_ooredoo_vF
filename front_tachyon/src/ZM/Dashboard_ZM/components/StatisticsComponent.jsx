...
const STTStatisticsComponent = ({ sttData, selectedDate, startDate, endDate }) => {
  ...
  const [showMap, setShowMap] = useState(false);
    ...
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await axios.get('http://localhost:3000/test-cron/sla-critiques');
    ...
    const processData = (type) => {
      if (sttData[type]?.details) {
        return {
          ...
          mapData: sttData[type]?.data?.filter(item => 
            item.LATITUDE_SITE && item.LONGITUDE_SITE && 
            item.LATITUDE_SITE !== 0 && item.LONGITUDE_SITE !== 0
          ) || []
        };
      }
      return null;
  ...
  useEffect(() => {
    if (allProcessedData) {
      ...
      if (currentData) {
        ...
        setMapData(currentData.mapData);
        setIsLoading(false);
      } else {
  ...
  const calculateMapCenter = () => {
    if (mapData.length === 0) return [34, 10]; // Default center for Tunisia
    
    const latSum = mapData.reduce((sum, item) => sum + item.LATITUDE_SITE, 0);
    const lngSum = mapData.reduce((sum, item) => sum + item.LONGITUDE_SITE, 0);
    
    return [latSum / mapData.length, lngSum / mapData.length];
  };
      ...
      {!chartData.length ? (
        ...
        <>
          ...
          {showMap && mapData.length > 0 && (
            ...
            >
              <h3>Localisation des interventions ({activeTab})</h3>
              <div className="map-container">
                ...
                >
                  ...
                  {mapData.map((item, index) => (
                    <Marker 
                      key={index} 
                      position={[item.LATITUDE_SITE, item.LONGITUDE_SITE]}
                    >
                      <Popup>
                        <div>
                          <h4>Intervention #{item.crm_case}</h4>
                          <p><strong>Client:</strong> {item.CLIENT}</p>
                          <p><strong>MSISDN:</strong> {item.MSISDN}</p>
                          <p><strong>STT:</strong> {item.NAME_STT}</p>
                          <p><strong>Date:</strong> {new Date(item.DATE_AFFECTATION_STT).toLocaleString('fr-FR')}</p>
                        </div>
                      </Popup>
                    </Marker>
                  ))}
