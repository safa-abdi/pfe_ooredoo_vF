...
const BatchMapPopup = ({
  ...
  useEffect(() => {
    if (isOpen && activations.length > 0) {
      fetchCommonSTTs();
    }
  }, [isOpen, activations]);
      ...
      await Promise.all(stts.map(async (stt) => {
        try {
          ...
          const response = await fetch(
            `${BASE_API_URL}/activation/stt/${sttId}/in-progress-count/${Gouv}`
          );
    ...
    try {
      await Promise.all(stts.map(async (stt) => {
        try {
          const sttId = stt.id;
          const response = await fetch(
            `${BASE_API_URL}/activation/stt/${sttId}/in-progress-count/${Gouv}/${createdDeleg}`
          );
...
const handleAssignClick = async () => {
  ...
  try {
    ...
    setErrorMessage(error.message || "Erreur lors de l'affectation");
    setTimeout(() => setErrorMessage(null), 4000);
  }
};
...
useEffect(() => {
  ...
  const fetchCommonSTTs = async () => {
    try {
      ...
      const selectedGovernorates = [...new Set(activations.map(a => a.Gouvernorat))];
      const selectedDelegations = [...new Set(activations.map(a => a.Delegation))];
  ...
  if (!isOpen || activations.length === 0) return null;

  const center = {
    lat: activations.reduce((sum, a) => sum + parseFloat(a.LATITUDE_SITE), 0) / activations.length,
    lng: activations.reduce((sum, a) => sum + parseFloat(a.LONGITUDE_SITE), 0) / activations.length
  };
            ...
            {isselectedGouv !== true ? (
              ...
              <>
                ...
                <div className="selection-count">
                  {activations.length} activation(s) sélectionnée(s)
                </div>
              </>
            )}
          </div>
            ...
            >
              ...
              {activations.map((activation) => (
                <Marker
                  ...
                  position={[
                    parseFloat(activation.LATITUDE_SITE),
                    parseFloat(activation.LONGITUDE_SITE)
                  ]}
                  icon={createCustomIcon('red')}
                  eventHandlers={{
                    click: (e) => {
                      e.originalEvent.stopPropagation();
                      setSelectedMarker(activation.crm_case);
                    }
                  }}
                >
                  {selectedMarker === activation.crm_case && (
                    <Popup
                      onClose={() => setSelectedMarker(null)}
                      className="custom-popup"
                    >
                      <div className="popup-content_deselect">
                        <h3>{activation.crm_case}</h3>
                        <div className="popup-details">
                          <p><strong>Client:</strong> {activation.CLIENT}</p>
                          <p><strong>Gouvernorat:</strong> {activation.Gouvernorat}</p>
                        </div>
                        <button
                          className="deselect-btn"
                          onClick={(e) => {
                            e.stopPropagation();
                            onDeselect(activation);
            ...
            <div className="stt-list-container">
              {loadingCounts ? (
                ...
                <div className="stt-select-wrapper">
                  ...
                  <button
                    className={`assign-btn ${isReassignment ? 'reassign' : ''}`}
                    onClick={handleAssignClick}
                    disabled={!selectedSTT}
                  >
                    {isReassignment ? 'Réaffecter' : 'Affecter'}
                  </button>
                </div>
              ) : (
