...
import '../styles/resiliationItem.css';
import MapPopup from '../../activation/component/MapPopup';
import { BASE_API_URL } from '../../../../config';

const ResiliationItem = ({ plainte, isSelected, onToggleSelect, onClick }) => {
  ...
  return (
    <div className={`resiliation-item ${isSelected ? 'selected' : ''}`}>
      <div className="resiliation-item-header">
        ...
        <div className="resiliation-item-msisdn">MSISDN: {plainte.MSISDN}</div>
      </div>


      <div className="resiliation-item-details">
        <p><FontAwesomeIcon icon={faUser} className="icon1" /><span>{plainte.CLIENT || NaN}</span></p>
        <p><FontAwesomeIcon icon={faPhoneAlt} className="icon1" /><span>{plainte.CONTACT1_CLIENT}</span></p>
        <p><FontAwesomeIcon icon={faMobileAlt} className="icon1" /><span>{plainte.CONTACT2_CLIENT}</span></p>
        <p><span>{plainte.STATUT}</span></p>
      </div>

      <div className="buttonItem">
        <div className="resiliation-item-gouvDel">
          <FontAwesomeIcon icon={faMapMarkerAlt} className="icon2" />
          <span>{plainte.Gouvernorat + " , " + plainte.Delegation || 'Non affecté'}</span>
        </div>

        <div className="resiliation-item-stt">
          <p><FontAwesomeIcon icon={faTowerCell} className="icon1" />&nbsp;<span>{plainte.STT ?? "N/A"}</span></p>
        </div>

        <div className="resiliation-item-cgrps" onClick={handleMapIconClick}>
          <span>
...
export default ResiliationItem;