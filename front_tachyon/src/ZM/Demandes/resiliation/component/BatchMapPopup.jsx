...
const BatchMapPopup = ({
  ...
  useEffect(() => {
    if (isOpen && resiliations.length > 0) {
      fetchCommonSTTs();
    }
  ...
  const loadSttProgressCounts = async (stts) => {
    ...
    try {
      await Promise.all(stts.map(async (stt) => {
        try {
          const sttId = stt.id;
          const response = await fetch(`${BASE_API_URL}/resiliation/stt/${sttId}/in-progress-count/${Gouv}`
          );
      ...
      await Promise.all(stts.map(async (stt) => {
        try {
          ...
          const response = await fetch(
            `${BASE_API_URL}/resiliation/stt/${sttId}/in-progress-count/${Gouv}/${createdDeleg}`
          );
  ...
  const fetchCommonSTTs = async () => {
    try {
      ...
      const selectedGovernorates = [...new Set(resiliations.map(a => a.Gouvernorat))];
      const selectedDelegations = [...new Set(resiliations.map(a => a.Delegation))];
  ...
  if (!isOpen || resiliations.length === 0) return null;

  const center = {
    lat: resiliations.reduce((sum, a) => sum + parseFloat(a.LATITUDE_SITE), 0) / resiliations.length,
    lng: resiliations.reduce((sum, a) => sum + parseFloat(a.LONGITUDE_SITE), 0) / resiliations.length
  };
            ...
            {isselectedGouv !== true ? (
              ...
              <>
                ...
                <div className="selection-count">
                  {resiliations.length} resiliation(s) sélectionnée(s)
                </div>
              </>
            )}
          </div>
            ...
            >
              ...
              {resiliations.map((resiliation) => (
                <Marker
                  ...
                  position={[
                    parseFloat(resiliation.LATITUDE_SITE),
                    parseFloat(resiliation.LONGITUDE_SITE)
                  ]}
                  icon={createCustomIcon('red')}
                  eventHandlers={{
                    click: (e) => {
                      e.originalEvent.stopPropagation();
                      setSelectedMarker(resiliation.crm_case);
                    }
                  }}
                >
                  {selectedMarker === resiliation.crm_case && (
                    <Popup
                      onClose={() => setSelectedMarker(null)}
                      className="custom-popup"
                    >
                      <div className="popup-content_deselect">
                        <h3>{resiliation.crm_case}</h3>
                        <div className="popup-details">
                          <p><strong>Client:</strong> {resiliation.CLIENT}</p>
                          <p><strong>Gouvernorat:</strong> {resiliation.Gouvernorat}</p>
                        </div>
                        <button
                          className="deselect-btn"
                          onClick={(e) => {
                            e.stopPropagation();
                            onDeselect(resiliation);
